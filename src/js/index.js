...
const logoUrl =
  "https://cdn.appscode.com/images/products/bytebuilders/bytebuilders-512x512.png";
...
function getModalHtml(domain, conf) {
  ...
  let url = `${domain[mode] || domain.prod}/`;
...
export function initialize(ref, conf) {
  ...
  btn.addEventListener("mouseenter", (e) => {
    handleButtonMouseEnter(e, btn);
  });
  btn.addEventListener("mouseleave", (e) => {
    handleButtonMouseLeave(e, btn);
  });
  // add event listener to listen to iframe messages
  window.addEventListener("message", (e) => {
    handleIframeMessages(e, btn, acModal);
  });
  // add event listener to handle keydown event
  btn.addEventListener("keydown", (e) => {
    handleKeyDown(e, acModal);
  });
}
